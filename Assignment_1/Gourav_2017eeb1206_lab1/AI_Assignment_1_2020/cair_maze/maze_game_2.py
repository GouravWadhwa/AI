...
from .pathfinding_2 import search_A_star
...
class MazeGame:
    ...
    def __init__(self, maze_size,
        ...
        self.sprite_target_1 = Sprite(color=(255, 0, 0), x=0, y=0, w=self.tile_width, h=self.tile_height)
        self.sprite_target_2 = Sprite(color=(255, 0, 0), x=0, y=0, w=self.tile_width, h=self.tile_height)
        self.sprites = pygame.sprite.LayeredUpdates(self.sprite_maze, [self.sprite_target_1, self.sprite_target_2, self.sprite_player])
        self.rectangles = []
        ...
        ##
        #############################################################
        self.player, self.target_1, self.target_2 = None, None, None
    ...
    def get_state(self, type=StateType.DEFAULT, resize=None):
        ...
        if type == StateType.ImageRGB or type == StateType.ImageGrayScale:
            ...
            state[self.target_1[0], self.target_1[1]] = 3
            state[self.target_2[0], self.target_2[1]] = 3
    ...
    def reset(self, type=StateType.DEFAULT):
        ...
        if self.options["disable_target"]:
            self.player, _, _ = self.spawn_players()
            self.target_1 = (-1, -1)
            self.target_2 = (-1, -1)
        else:
            # Set player positions
            self.player, self.target_1, self.target_2 = self.spawn_players()

            # Calculate shortest path

            self.maze_optimal_path = search_A_star(self, self.player, self.target_1, self.target_2)
            self.maze_optimal_path_length = self.maze_optimal_path[0]

        # Update player sprites
        self.sprite_player.move(*self.player)
        self.sprite_target_1.move(*self.target_1)
        self.sprite_target_2.move (*self.target_2)
    ...
    def spawn_players(self):
        """
        Spawns the players at three "random" locations
        :return:
        """
        start_positions = []
        for start_position in [(0, 0), (random.randint (0, self.width - 1), random.randint (0, self.height - 1)), (random.randint (0, self.width - 1), random.randint (0, self.height - 1))]:
            queue = deque()
    ...
    def step(self, a, type=StateType.DEFAULT):
        ...
        if self.player == self.target_1 or self.player == self.target_2:
            self.terminal = True
