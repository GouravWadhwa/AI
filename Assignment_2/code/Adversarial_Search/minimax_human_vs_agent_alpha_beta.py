...
def max_value (problem, board, alpha, beta) :
    ...
    for child in childs :
        v = max (v, min_value (problem, child, alpha, beta))
        if v >= beta :
            return v
        alpha = max (alpha, v)
...
def min_value (problem, board, alpha, beta) :
    if problem.goal_test (board) != None :
        return problem.goal_test (board)
    ...
    for child in childs :
        v = min (v, max_value (problem, child, alpha, beta))
        if v <= alpha :
            return v
        beta = min (beta, v)
...
if __name__ == '__main__' :
    ...
    while (True) :
        ...
        value = -2
        for child in childs :
            new_value = min_value (problem, child, -2, 2)
            if new_value > value :
                board = child
                value = new_value
